...
class Intelligence(Resource):
    ...
    class Domain(Resource):
        ...
        def get_campaigns(self, domain, **kwargs):
            ...
            the sendingIps embed option. Embed parameter accepts: sendingIps, rawEmail, ispPlacements, links,
            headers (headers is only available for ESP-owned accounts). Order options: firstSeen, -firstSeen,
            lastSeen, -lastSeen, inbox, -inbox, spam, -spam
        ...
        def get_campaigns_per_week(self, domain, **kwargs):
            """
            Find the campaign average per week for given domain, default parameters and startDate=now&weeksBack=4
        ...
        def get_average_volume_per_campaign(self, domain, **kwargs):
            """
            Find the average campaign volume for a domain over a given time period, default daysBack:28
